...
# Open netcdf file
nc_file = netCDF4.Dataset('carma_volfalltest.nc', 'r')
...
for it, i in zip(range(0, len(time), int(len(time)/5)), range(5)):
    axs[0].plot(total_mass[it,:], km, color=colors[i], label=str(int(time[it])) + ' years')
...
plt.tight_layout()
plt.savefig('read_volfalltest.png')
plt.show()
